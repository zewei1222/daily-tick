...
四個分頁：對戰／背包／任務／統計。App 永遠啟動在「任務」——工具不被遊戲劫持。
...
| 主題 | 落點 |
|---|---|
| 內容註冊表 | `js/game/content.js` —— 標籤、實體類型、稀有度、道具目錄、怪物模板、平衡常數全在這，**是資料不是程式碼**；新增標籤/道具/類型只要加表（戰鬥行為在 `battle.js` 的 `TAG_HANDLERS` 加一個 handler） |
...
| 戰鬥 | `js/game/battle.js` —— 回合制、速度 carry 出手制（比 1.5 → 出手 1,2,1,2）、單回合出手上限 10、100 回合強制判敗；先完整模擬再回放，可跳過；怪物由公式生成＋從標籤表隨機抽 1–2 個詞綴（resist 削弱玩家該系效果 / counter 怪物反用該效果） |
